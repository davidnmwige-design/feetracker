...
export async function POST(req: Request) {
  ...
  if (!hasPermission(role, 'students', 'POST')) return NextResponse.json(FORBIDDEN, { status: 403 })
  ...
  const existing = await prisma.student.findFirst({ where: { admNo: cleanAdmNo, schoolId: user.school.id } })
